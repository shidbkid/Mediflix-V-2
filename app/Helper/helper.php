...
if (!function_exists('updateManifest')) {
    ...
    {
        $manifest = [
            "name" => get_option('app_name', 'Trivio'),
            "short_name" => get_option('app_name', 'Trivio'),
            "start_url" => route('main.index'),
            "background_color" => get_option('app_theme_color', '#5e3fd7'),
            "description" => get_option('app_name', 'Trivio'),
            "display" => "fullscreen",
...
if (!function_exists('isEnableOpenAI')) {
    ...
    {
        $role = auth()->user()?->role;
        return isAddonInstalled('TrivioAI') && in_array($role, [USER_ROLE_INSTRUCTOR, USER_ROLE_ORGANIZATION, USER_ROLE_ADMIN]) && get_option('open_ai_system');
    }
}
